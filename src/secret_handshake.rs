...
pub fn actions(n: u8) -> Vec<&'static str> {
    let mut res: Vec<&'static str> = ACTION.iter().enumerate()
        .filter(|(idx, _)| n & (1 << idx) > 0)
        .map(|(_, action)| *action)
        .collect();
    if n & (1 << 4) > 0 {
        res.reverse();
...
mod secret_handshake_test {
    use crate::secret_handshake::actions;
